...
dates_table <- read.table("~/Documents/Universitat/Holanda/Projecte/final_dates.txt", sep = "\t", header = T, row.names = 1, check.names = F)



#tax_level_table: taxonomy table in a concrete level (species, genus...)
#pcoa_elements: number of elements to calculate in the pcoa analysis
#variable_table: table to color the pcoa plot
#top_value: number of the most abundant variants to color the different plots (if the variable_table is numeric, if not: top_value = 1)

pcoa_function(species_table, 4, filum_table, 3)
